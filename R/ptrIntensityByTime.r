...
#        result=apply(tofSubset,2:3,sum)
      ...
      df=data.frame("time"=Time,"intensity"=as.vector(result))
    }
    if(length(mz)==2)
    {
      tofSubset=tofData[which(MassAxis>mz[1]&MassAxis<mz[2]),,]
...
#      message(paste0(i,"/",dim(integrationTable)[1]))
      ...
      dfi=data.frame("time"=Time,"intensity"=as.vector(sumSubset),"mz"=integrationTable[i,"mz"],name=integrationTable[i,"name"])
      df=rbind(df,dfi)
    }
  }
  res=list(df=df)
