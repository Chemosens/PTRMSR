...
ptrvListIntensityByTime=function(listFiles,timeCol="RelTime",colToRemove=c("AbsTime","Cycle"),removeBlankTime=FALSE,ions=NULL,dec_vec=rep(".",length(listFiles)),sep="\t",metaData=NULL,correction="cycle",timeBlank=c(0,30),halfWindowSize=5,method="MAD",total=FALSE,breathRatio=FALSE,stat="area",minimalDuration=2,smoothMethod="MovingAverage",minExpi=NULL,maxInspi=NULL,forMinExpiDivideMaxIntBy=5,forMaxInspiDivideMaxIntBy=4)
{
  ...
  for(i in 1:length(listFiles))
  {
   ...
   metaInfo=metaData[metaData[,"file"]==file,]
    result_all=ptrvIntensityByTime(dataset=dataset,timeCol=timeCol, colToRemove=colToRemove,ions=ions,referenceBreath=metaInfo[1,"resp"],timeStart=0,correction=correction,timePeriod=c(0,metaInfo[1,"finPTR_s"]),removeNoise=TRUE,timeBlank = c(0,metaInfo[1,"miseEnBouche_s"]),halfWindowSize=halfWindowSize,method=method,total=total,breathRatio=breathRatio,smoothMethod=smoothMethod,minimalDuration=minimalDuration,minExpi=minExpi,maxInspi=maxInspi,forMinExpiDivideMaxIntBy=forMinExpiDivideMaxIntBy,forMaxInspiDivideMaxIntBy=forMaxInspiDivideMaxIntBy)
    cycleLimits[[i]]=result_all$p_cyclelimits
