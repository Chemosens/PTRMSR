...
#    ii. Smooth data
p=plot(int_time_IT,spar=0.5,groupOption="name",main="Intensity accross time",legend=FALSE)
ggplotly(p)

#    iii. Some ions, for specific interval
p=plot(int_time_IT,selec="Propanol",xlim=c(40,200))
ggplotly(p)
...
listPTRM=ptrReadListIBM(listFiles,sumSpectraOnly=TRUE,normalization="none",rt=NULL,integrationTable=NULL,breaks=breaks,by=0.01)

# TODO
#plot(listPTRM,groupOption="file")

p=plot_fullspectra(listPTRM[,"mz"],listPTRM[,"intensity"],listPTRM[,"file"])

# Visualisation par fichier et temps
listPTRT=ptrReadListIBT(listFiles,normalization="none",rt=NULL,integrationTable="device",breaks=breaks,by=0.1,mz=NULL)
  

dfbut=listPTRT[listPTRT[,"name"]=="Butanone",]


p=plot_fullspectra(dfbut[,"time"],dfbut[,"intensity"],dfbut[,"file"])
