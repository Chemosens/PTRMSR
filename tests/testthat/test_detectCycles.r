...
cycles=detectCycle(df=res1,maxPeaks=NULL,smoothMethod="MovingAverage",method="MAD",halfWindowSize=12,maximum=NULL,SNR=0,minimalDuration=1)


test_that("cycles avec duration minimale 1",expect_true(sum(diff(cycles$cycles)<1)==0))


cycles=detectCycle(df=res1,maxPeaks=NULL,smoothMethod="MovingAverage",method="MAD",halfWindowSize=5,maximum=NULL,SNR=0,minimalDuration=2)
